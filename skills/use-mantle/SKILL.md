...
1. Discover the connected tools and current schemas. Names may have a client-added prefix. Use only available tools; consult [the tool reference](../../docs/tools.md) for operation-specific behavior.
...
7. For agent draft edits, follow [edit-mantle-agent](../edit-mantle-agent/SKILL.md). Report completed changes, remaining approval steps, or safe error details.
...
Report bounded `code` and `request_id` values rather than raw errors. Honor `retry_after` / `Retry-After`. Stop repeated authorization failures and ask the user to reconnect. Do not claim access to hidden instructions, credentials, task prompts, task-run output, or capabilities absent from the live schema.
